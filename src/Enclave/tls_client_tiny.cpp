...
{
  (void)opaque;
  LL_DEBUG("(%d, %d) %s: %s", nkey, nvalue, ckey, cvalue);
}
...
void HttpsClient::sendRequest()
{
  ...
  for (unsigned written = 0, frags = 0; written < requestMessage.size();
       written += ret, frags++) {
...
HttpResponse HttpsClient::getResponse()
{
   ...
   */
  LL_DEBUG("connecting over TCP: %s:%s...",
  ...
  LL_DEBUG("Hand shake succeeds: [%s, %s]",
  ...
  while (http_need_more) {
    ...
    */
    ...
    while (http_need_more && n_data) {
      ...
      n_data -= read;
      data += read;
      LL_DEBUG("http_need_more=%d, n_data=%d", http_need_more, n_data);
    }
  }  // while (http_need_more)
